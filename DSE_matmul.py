...
if __name__ == "__main__":
  ...
  designs_to_synthesise = [
    DesignConfig("matmul", 
                 S_q=32, S_kv=32, d_kq=32, d_v=32, # Standard size
                 k=k, 
                 scale_width=8, 
                 M1_E=exp, M1_M=man, 
                 M2_E=exp, M2_M=man, 
                 M3_E=exp, M3_M=man,
                 accum_method1=accum_method,
                 m1_dsp=dsp)
    for k in [4, 8, 16] 
    for exp, man in [(4, 3), (5, 2), (5, 10)] # Various Precisions
    for accum_method in [AccumMethod.Kulisch, AccumMethod.Kahan, AccumMethod.Neumaier, AccumMethod.TwoSum]
    for dsp in ["yes", "no"]
  ]
