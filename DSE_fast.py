...
class SynthesisHandler:
  ...
  def check_if_design_is_invalid(self, design):
    ...
    for param in [design.S_q, design.S_kv, design.d_kq, design.d_v, design.k1, design.k2, design.k3, design.scale_width]:
      if param <= 0:
        return True
    ...
    # d_kq and d_v must be divisible by k
    if design.d_kq % design.k2 != 0 or design.d_v % design.k2 != 0:
      return True
      
    # S_kq and S_v must be divisible by k
    if design.S_q % design.k2 != 0 or design.S_kv % design.k2 != 0:
      return True

    # d_kq and d_v must be divisible by k
    if design.d_kq % design.k3 != 0 or design.d_v % design.k3 != 0:
      return True
      
    # S_kq and S_v must be divisible by k
    if design.S_q % design.k3 != 0 or design.S_kv % design.k3 != 0:
      return True
  ...
  def run_accuracy_measurement(self, gpus=[0], dry_run=False, verbose=False):
    if not self.designs_to_synthesise:
      print("No designs to measure accuracy for specified.")
      return

    # Filter out designs that already have results
    designs_to_run = []
    for design in self.designs_to_synthesise:
      if self.check_if_result_exist(design, "_accuracy.txt"):
        ...
        continue
      designs_to_run.append(design)

    if not designs_to_run:
      print("All designs already have accuracy reports.")
      return

    print(f"Running accuracy measurement for {len(designs_to_run)} designs on {len(gpus)} GPUs: {gpus}...")

    chunks = [[] for _ in range(len(gpus))]
    for i, design in enumerate(designs_to_run):
      chunks[i % len(gpus)].append(design)

    def worker(gpu_id, designs):
      print(f"GPU {gpu_id} started with {len(designs)} designs.")
      for design in designs:
        ...
        if verbose:
          print(f"[GPU {gpu_id}] Running accuracy measurement for {design!r}...")
        
        if not dry_run:
          self._generate_accuracy_report(design, accuracy_report_path, gpu_id)
        else:
           print(f"[GPU {gpu_id}] Dry run: would run for {design!r}")
      print(f"GPU {gpu_id} finished.")

    threads = []
    for i, gpu_id in enumerate(gpus):
      if chunks[i]: # Only start thread if there is work
        t = threading.Thread(target=worker, args=(gpu_id, chunks[i]))
        t.start()
        threads.append(t)
  ...
  def _generate_accuracy_report(self, design, accuracy_report_path, gpu_id):
    accuracy_cmd = f"CUDA_VISIBLE_DEVICES={gpu_id} python -u bert/llama_ppl.py {design.get_bert_flags()}"
    print(accuracy_cmd)
...
if __name__ == "__main__":
  ...
  synthesis_handler = SynthesisHandler(designs_to_synthesise, synth_output_dir="synth_output_matmul", max_workers=16)  
  for i in range(50):
  ...
  designs_to_synthesise = [
    DesignConfig(name, S, S, d, d, k, k, k, scale_width, M1_E, M1_M, M1_E, M1_M, M1_E, M1_M, accum_method_1, accum_method_1, accum_method_1, m1_dsp, m1_dsp, m1_dsp)
  ...
  synthesis_handler = SynthesisHandler(designs_to_synthesise, synth_output_dir="synth_output_softmax", max_workers=16)
  for i in range(50):
    synthesis_handler.run_synthesis(dry_run=args.dry, verbose=args.verbose)
    print(f"========================================================\n========================================================\n========================================================\n SOFTMAX RUN {i}\n========================================================\n========================================================\n========================================================\n")
