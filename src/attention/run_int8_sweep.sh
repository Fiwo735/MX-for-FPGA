...
# Options: "yes" (DSP) and "logic" (No DSP)
OPTIONS=("yes" "logic")

for m1 in "${OPTIONS[@]}"; do
  for sm in "${OPTIONS[@]}"; do
    for m2 in "${OPTIONS[@]}"; do
      # Skip the known all-logic case
      if [ "$m1" == "logic" ] && [ "$sm" == "logic" ] && [ "$m2" == "logic" ]; then
          continue
      fi
      run_test "$m1" "$sm" "$m2"
    done
...
echo "Sweep completed. Results in $SUMMARY_FILE"
