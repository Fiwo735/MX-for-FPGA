...
module kahan_adder_tree #(
    ...
    for(genvar i=0; i<TREE_DEPTH; i++) begin : top_tree
        ...
        logic signed [BIT_WIDTH_I+i-1:0] c_a     [ELEMS_COUNT>>(1+i)];
        logic signed [BIT_WIDTH_I+i-1:0] sum_b   [ELEMS_COUNT>>(1+i)];
        logic signed [BIT_WIDTH_I+i-1:0] c_b     [ELEMS_COUNT>>(1+i)];

        logic signed [BIT_WIDTH_I+i-1:0] sum_res [ELEMS_COUNT>>(1+i)];
        logic signed [BIT_WIDTH_I+i-1:0] c_res   [ELEMS_COUNT>>(1+i)];
        ...
        if(i != 0) begin
            ...
            for(genvar j=0; j<(ELEMS_COUNT>>2); j++) begin : kahan_starts
                kahan_start #(
                    ...
                    .clk_i(clk_i), // Not used in this context
                    .rst_ni(rst_ni), // Not used in this context
                    .e0(i_vec_reg[4*j]),
                    .e1(i_vec_reg[4*j+1]),
                    .e2(i_vec_reg[4*j+2]),
                    .e3(i_vec_reg[4*j+3]),
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            o_sum <= top_tree[TREE_DEPTH-1].sum_res[0][SUM_WIDTH_O-1:0]; // Truncate to SUM_WIDTH_O
        end
    end
