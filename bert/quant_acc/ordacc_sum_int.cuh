...
__global__ void ordacc_chunk_comp_sum_scaled_int_kernel(
    const scalar_t* __restrict__ input,
    const float* __restrict__ scale_input,
    float* __restrict__ output,
    int batch_size, int reduce_dim,
    int bit_w, int group_size
){
    ...
    for (int k = 0; k < reduce_dim; ++k){
        ...
        y_inner = round_rne_int(value_inner - c_inner, bit_w);
        t_inner = round_rne_int(sum_inner + y_inner, bit_w);
        c_inner = round_rne_int(t_inner - sum_inner, bit_w) - y_inner;
        c_inner = round_rne_int(c_inner, bit_w);
        sum_inner = round_rne_int(t_inner, bit_w);
...
template <typename scalar_t>
__global__ void ordacc_chunk_2sum_scaled_int_kernel(
    const scalar_t* __restrict__ input,
    const float* __restrict__ scale_input,
    float* __restrict__ output,
    int batch_size, int reduce_dim,
    ...
    for (int k = 0; k < reduce_dim; ++k){
        ...
        s_inner = round_rne_int(sum_inner + value_inner, bit_w);
        sum_p_inner = round_rne_int(s_inner - value_inner, bit_w);
        value_p_inner = round_rne_int(s_inner - sum_p_inner, bit_w);
        d_sum_inner = round_rne_int(sum_inner - sum_p_inner, bit_w);
        d_value_inner = round_rne_int(value_inner - value_p_inner, bit_w);
        d_added_inner = round_rne_int(d_sum_inner + d_value_inner, bit_w);
        error_inner = round_rne_int(error_inner + d_added_inner, bit_w);
        sum_inner = s_inner;

        if ((k + 1) % ROUND_INTERVAL == 0 || k == reduce_dim - 1){
            value_outer = round_rne_int(sum_inner + error_inner, bit_w);
            sum_inner = 0;
            error_inner = 0;
...
template <typename scalar_t>
__global__ void ordacc_chunk_fast2sum_scaled_int_kernel(
    const scalar_t* __restrict__ input,
    const float* __restrict__ scale_input,
    float* __restrict__ output,
    int batch_size, int reduce_dim,
    ...
    for (int k = 0; k < reduce_dim; ++k){
        ...
        s_inner = round_rne_int(sum_inner + value_inner, bit_w);
        z_inner = round_rne_int(s_inner - sum_inner, bit_w);
        val_z_sub_inner = round_rne_int(value_inner - z_inner, bit_w);
...
template <typename scalar_t>
__global__ void ordacc_chunk_neumaier_scaled_int_kernel(
    const scalar_t* __restrict__ input,
    const float* __restrict__ scale_input,
    float* __restrict__ output,
    int batch_size, int reduce_dim,
    ...
    for (int k = 0; k < reduce_dim; ++k){
        ...
        c_inner += (fabsf(sum_inner) >= fabsf(value_inner)) ?
            round_rne_int(round_rne_int(sum_inner - s_inner, bit_w) + value_inner, bit_w):
            round_rne_int(round_rne_int(value_inner - s_inner, bit_w) + sum_inner, bit_w);
        c_inner = round_rne_int(c_inner, bit_w);
        sum_inner = s_inner;

        if ((k + 1) % ROUND_INTERVAL == 0 || k == reduce_dim - 1){
            value_outer = round_rne_int(sum_inner + c_inner, bit_w);
            sum_inner = 0;
            c_inner = 0;
...
template <typename scalar_t>
__global__ void ordacc_chunk_klein_scaled_int_kernel(
    const scalar_t* __restrict__ input,
    const float* __restrict__ scale_input,
    float* __restrict__ output,
    int batch_size, int reduce_dim,
    ...
    for (int k = 0; k < reduce_dim; ++k){
        ...
        if ((k + 1) % ROUND_INTERVAL == 0 || k == reduce_dim - 1){
            value_outer = round_rne_int(sum_inner + round_rne_int(cs_inner + ccs_inner, bit_w), bit_w);
            sum_inner = 0;
            cs_inner = 0;
            ccs_inner = 0;
...
template <typename scalar_t>
__global__ void ordacc_chunk_full_quant_scaled_int_kernel(
    const scalar_t* __restrict__ input,
    const float* __restrict__ scale_input,
    float* __restrict__ output,
    int batch_size, int reduce_dim,
    ...
    for (int k = 0; k < reduce_dim; ++k){
        ...
        sum_inner = round_rne_int(sum_inner + scaled_product, bit_w);
...
torch::Tensor ordacc_chunk_scaled_int(
    torch::Tensor input,
    ...
    if (sum_type == "QUANT"){
        AT_DISPATCH_INTEGRAL_TYPES(input_flat.scalar_type(), "ordacc_chunk_scaled_quant", ([&]{
            ordacc_chunk_full_quant_scaled_int_kernel<scalar_t><<<grid_dim, block_dim>>>(
        ...
        AT_DISPATCH_INTEGRAL_TYPES(input_flat.scalar_type(), "ordacc_chunk_scaled_kahan", ([&]{
            ordacc_chunk_comp_sum_scaled_int_kernel<scalar_t><<<grid_dim, block_dim>>>(
                input_flat.data_ptr<scalar_t>(),
                scale_input_flat.data_ptr<float>(),
                output.data_ptr<float>(),
                rows,
        ...
        AT_DISPATCH_INTEGRAL_TYPES(input_flat.scalar_type(), "ordacc_chunk_scaled_2sum", ([&]{
            ordacc_chunk_2sum_scaled_int_kernel<scalar_t><<<grid_dim, block_dim>>>(
                input_flat.data_ptr<scalar_t>(),
                scale_input_flat.data_ptr<float>(),
                output.data_ptr<float>(),
                rows,
        ...
        AT_DISPATCH_INTEGRAL_TYPES(input_flat.scalar_type(), "ordacc_chunk_scaled_fast2sum", ([&]{
            ordacc_chunk_fast2sum_scaled_int_kernel<scalar_t><<<grid_dim, block_dim>>>(
                input_flat.data_ptr<scalar_t>(),
                scale_input_flat.data_ptr<float>(),
                output.data_ptr<float>(),
                rows,
        ...
        AT_DISPATCH_INTEGRAL_TYPES(input_flat.scalar_type(), "ordacc_chunk_scaled_neumaier", ([&]{
            ordacc_chunk_neumaier_scaled_int_kernel<scalar_t><<<grid_dim, block_dim>>>(
                input_flat.data_ptr<scalar_t>(),
                scale_input_flat.data_ptr<float>(),
                output.data_ptr<float>(),
                rows,
        ...
        AT_DISPATCH_INTEGRAL_TYPES(input_flat.scalar_type(), "ordacc_chunk_scaled_klein", ([&]{
            ordacc_chunk_klein_scaled_int_kernel<scalar_t><<<grid_dim, block_dim>>>(
                input_flat.data_ptr<scalar_t>(),
                scale_input_flat.data_ptr<float>(),
                output.data_ptr<float>(),
                rows,
