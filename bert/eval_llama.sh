...
for v in "${v_values[@]}"; do
    for sum_type in "${sum_types[@]}"; do
        for c in "${c_values[@]}"; do
            ...
            # k_config=$(printf '{"quant":"MXFPQuantizer","man_w":2,"exp_w":4,"group_size":%d}' "$c")
            # s_config=$(printf '{"quant":"MXFPQuantizer","man_w":2,"exp_w":4,"group_size":%d}' "$c")
            # v_config=$(printf '{"quant":"MXFPQuantizer","man_w":6,"exp_w":4,"group_size":%d}' "$c")
            ...
            CUDA_VISIBLE_DEVICES=2 python llama_ppl.py \
