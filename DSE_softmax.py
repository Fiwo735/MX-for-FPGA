...
class DesignConfig:
  ...
  def get_bert_flags(self):
    # Not used for component DSE but kept for structure compatibility
    return ""
  ...
  def __str__(self):
    ...
    s += f"  M2 bits: {self.M2_bits} (Output Precision)\n"
    # M3 unused for softmax isolation usually
    s += f"  Accumulation method: {self.accum_method2.value}\n"
    return s
...
class SynthesisHandler:
  def __init__(self, designs_to_synthesise=None, hdl_dir="./src/attention/", clock_period_ns=5):
    ...
    self.board_max_freq = 1000 # MHz - Loosened check for component level
    
    # Updated output dir for Softmax
    self.synth_output_dir = os.path.join(self.hdl_dir, "synth_output_softmax")
  ...
  def get_available_fpga_resources(key=None):
    # Device: xcv80
    # Approximate or Placeholder values if exact V80 not known in this context
    # Using previous ones for consistent percentage calculation, or update if V80 spec known
  ...
  def check_if_design_is_invalid(self, design):
    return False # Relax checks for component level
  ...
  def run_synthesis(self, dry_run=False, verbose=False):
    ...
    for design in self.designs_to_synthesise:
      ...
      # Point to new script
      run_synth_path = os.path.join(self.hdl_dir, "run_synth_softmax.tcl")
...
if __name__ == "__main__":
  parser = ArgumentParser(description='Run DSE for Softmax module assignment')
  ...
  log_filename = f"DSE_run_softmax_{timestamp}.log"
  ...
  print(f"Starting DSE for Softmax at {timestamp}")
  print(f"Logging to: {log_filename}\n")
  
  # Define Softmax Configurations
  # Softmax uses Accumulator Method 2 primarily (Internal Sum)
  designs_to_synthesise = [
    DesignConfig("softmax", S_q=1, S_kv=1, d_kq=1, d_v=1, k=k, 
                 scale_width=8, 
                 # Precisions derived in TCL script based on total width 
                 # Passing width index as M1/M2/M3 params to be consistent 
                 M1_E=exp, M1_M=man, 
                 M2_E=exp, M2_M=man, 
                 M3_E=exp, M3_M=man,
                 accum_method2=accum_method,
                 m2_dsp=dsp)
    for k in [2, 4, 8, 16] # Softmax parallelism
    for exp, man in [(4, 3), (5, 2), (5, 10)] # Various Precisions
    for accum_method in [AccumMethod.Kulisch, AccumMethod.Kahan, AccumMethod.Neumaier]
    for dsp in ["yes", "no"]
  ]
